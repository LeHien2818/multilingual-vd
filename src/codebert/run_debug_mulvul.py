...
from model_mulvul import Model
...
def convert_examples_to_features(js,tokenizer,args):
    ...
    language_map = {"Python": 0, "CCPP": 1}
    language_id = language_map.get(js.get('language',''), 0)
...
class TextDataset(Dataset):
    def __init__(self, tokenizer, args, file_path=None, test=False):
        self.examples = []
    ...
    def __getitem__(self, i):       
        return torch.tensor(self.examples[i].input_ids),torch.tensor(self.examples[i].label), torch.tensor(self.examples[i].language)
...
def train(args, train_dataset, model, tokenizer):
    ...
    for idx in range(args.num_train_epochs): 
        ...
        for step, batch in enumerate(bar):
            ...
            languages = batch[2].to(args.device) if len(batch) > 2 else None 
            model.train()
            loss,logits = model(inputs,labels, language=languages)
        ...
        results = evaluate(args, model, tokenizer, test=False)
...
def evaluate(args, model, tokenizer, test=False):
    ...
    eval_dataset = TextDataset(tokenizer, args,args.eval_data_file, test=test)
...
def test(args, model, tokenizer, test=True):
    # Note that DistributedSampler samples randomly
    eval_dataset = TextDataset(tokenizer, args,args.test_data_file, test=test)
    eval_sampler = SequentialSampler(eval_dataset)
...
def main():
    ...
    if args.do_eval:
        ...
        model.to(args.device)
        result=evaluate(args, model, tokenizer, test=False)
