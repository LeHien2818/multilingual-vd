    ...
    with torch.no_grad():
        for code_batch, vuln, cwe, cluster_type, languages in data_loader:
            ...
            for i in range(len(cwe)):
                ...
                # Special fallback: if routed to expert 1 but confidence is too low,
                # use expert 2 probability for this sample.
                if routed_id == 1 and (routed_id + 1) < expert_probs_all.shape[1]:
                    reference_sample_prob = float(expert_probs_all[i, routed_id + 1].item())
                    if sample_prob > 0.4 and sample_prob < 0.6 and reference_sample_prob >= 0.9:
                        sample_prob = reference_sample_prob
...
def run_pipeline():
    ...
    test_probabilities = []  # Final general routed predictions
    test_labels = []
    test_cwe_labels = []
    ...
    test_codes = []  
    test_cluster_types = []
    test_expert_prob_rows = []  # Per-sample probs for all experts
    ...
    with torch.no_grad():
        for code_batch, vuln, cwe, cluster_type, languages in test_loader:
            ...
            # Final logits are general routed outputs from router-selected experts
            logits, _, _, router_logits, _ = model(
                emb,
                raw_input_ids=input_ids,
                raw_attention_mask=attention_mask,
